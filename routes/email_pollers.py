                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in AVA (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in AVA" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "ava_kind" if "ava_kind" in cols else "'scheduled' AS ava_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                ava_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-AVA-Origin"] = "ava-ui"
                outer["X-AVA-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", ava_kind or "scheduled")[:64]
                outer["X-AVA-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `ava-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `AVA_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `ava-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("AVA_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `AVA_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (AVA_INPROCESS_POLLERS=0); "
            "drive `ava-mail poll-scheduled` externally."
        )
        return
    import asyncio
