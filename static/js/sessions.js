...
let _sortMode = Storage.get('ava-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'AVA Chat';
...
const FOLDER_STATE_KEY = 'ava-folder-state';
const FOLDER_ORDER_KEY = 'ava-folder-order';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to AVA kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'AVA Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('ava-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('ava-session-sort', mode);
  else Storage.remove('ava-session-sort');
  renderSessionList();
}
